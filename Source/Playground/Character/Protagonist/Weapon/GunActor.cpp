...
void AGunActor::AttackInputStarted()
{
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			...
			const APlayerController* PlayerController = Cast<APlayerController>(OwnerProtagonist->GetController());
			...
			if (Arrow != nullptr || PlayerController != nullptr)
			{
				...
				const auto ControllerRot = OwnerProtagonist->GetControlRotation();

				const auto ProjectileRot = OwnerProtagonist->GetActorRotation().Add(ControllerRot.Pitch +ProjectilePitchOffset,0,0);
	...
	{
		UGameplayStatics::PlaySoundAtLocation(this, FireSound, OwnerProtagonist->GetActorLocation());
	}
